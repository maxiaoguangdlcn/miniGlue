 ...
 * license.
 *
 * Update Date: November 12th, 2014
 *
...
type
  { TigBrightContrastLayer }

  TigBrightContrastLayer = class(TigCustomLayer)
  private
...
constructor TigBrightContrastLayer.Create(AOwner: TigLayerList;
  const ALayerWidth, ALayerHeight: Integer);
begin
...
procedure TigBrightContrastLayer.SetBrightAmount(AValue: Integer);
begin
  FBrightAmount := Clamp(AValue, MIN_CHANGE_AMOUNT, MAX_CHANGE_AMOUNT);
end;

procedure TigBrightContrastLayer.SetContrastAmount(AValue: Integer);
begin
  FContrastAmount := Clamp(AValue, MIN_CHANGE_AMOUNT, MAX_CHANGE_AMOUNT);
end;

procedure TigBrightContrastLayer.LayerBlend(F: TColor32;

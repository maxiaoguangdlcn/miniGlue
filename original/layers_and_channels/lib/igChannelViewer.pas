...
type
  TigChannelPanelArea = (cpaUnknown,
                         cpaVisibleMark,
                         cpaChannelThumbnail,
                         cpaChannelCaption);

  { TigChannelPanelCustomTheme }

  TigChannelPanelCustomTheme = class(TObject)
  private
  ...
  protected
    ...
    function GetPanelAreaAtXY(AChannel: TigCustomChannel;
      const APanelRect: TRect; const AX, AY: Integer): TigChannelPanelArea; virtual; abstract;
  public
    constructor Create;

    procedure Paint(ABuffer: TBitmap32; AChannel: TigCustomChannel;
      const ARect: TRect); virtual; abstract;

    function GetSnapshot(AChannel: TigCustomChannel;
      const AWidth, AHeight: Integer): TBitmap32; virtual; abstract;

    property ObjectSpan : Integer read FObjectSpan write SetObjectSpan;
  end;

  { TigChannelPanelStdTheme }

  TigChannelPanelStdTheme = class(TigChannelPanelCustomTheme)
  private
  ...
  protected
    ...
    function GetPanelAreaAtXY(AChannel: TigCustomChannel;
      const APanelRect: TRect; const AX, AY: Integer): TigChannelPanelArea; override;
  ...
  TigScrollChannelPanelThread = class; // forward declaration

  TigChannelViewer = class(TCustomPaintBox32)
  private
    FChannelManager          : TigCustomChannelManager;     // pointer to a channel manager
    FPanelTheme              : TigChannelPanelCustomTheme;  // render channel panels on the manager in this theme
    ...
    FScrollThread            : TigScrollChannelPanelThread;
    ...
    function MoveAlphaChannelInList(const ACurIndex, ANewIndex: Integer): Boolean;
    ...
    procedure SetChannelManager(const AValue: TigCustomChannelManager);
  ...
  public
    ...
    property ChannelManager          : TigCustomChannelManager read FChannelManager          write SetChannelManager;
    property OnMouseRightButtonClick : TNotifyEvent            read FOnMouseRightButtonClick write FOnMouseRightButtonClick;
  end;

  { TigScrollChannelPanelThread }

  TigScrollChannelPanelThread = class(TThread)
  private
    FChannelViewer : TigChannelViewer;
  protected
    procedure Execute; override;
  public
    constructor Create(AChannelViewer: TigChannelViewer);
  end;
...
constructor TigChannelPanelCustomTheme.Create;
begin
  inherited;
...
procedure TigChannelPanelCustomTheme.SetObjectSpan(AValue: Integer);
begin
...
constructor TigChannelPanelStdTheme.Create;
begin
  inherited;
...
destructor TigChannelPanelStdTheme.Destroy;
begin
...
procedure TigChannelPanelStdTheme.DrawPanelBorder(ABuffer: TBitmap32;
  const ARect: TRect);
begin
...
procedure TigChannelPanelStdTheme.DrawChannelVisibleIcon(ABuffer: TBitmap32;
...
function TigChannelPanelStdTheme.GetChannelVisibleIconRect(
...
function TigChannelPanelStdTheme.GetPanelAreaAtXY(AChannel: TigCustomChannel;
  const APanelRect: TRect; const AX, AY: Integer): TigChannelPanelArea;
var
  LRect      : TRect;
  LTestPoint : TPoint;
  LSpan2     : Integer;
...
function TigChannelPanelStdTheme.GetSnapshot(AChannel: TigCustomChannel;
...
procedure TigChannelPanelStdTheme.Paint(ABuffer: TBitmap32;
  AChannel: TigCustomChannel; const ARect: TRect);
var
  LRect         : TRect;
  LSize         : TSize;
...
constructor TigChannelViewer.Create(AOwner: TComponent);
begin
  inherited;
  ...
  FPanelTheme := TigChannelPanelStdTheme.Create;
...
destructor TigChannelViewer.Destroy;
begin
  ScrollThreadStop;
...
function TigChannelViewer.CanScrollDown: Boolean;
begin
  Result := FViewportOffset.Y + FWorkSize.Y > Self.ClientHeight;
end;

function TigChannelViewer.CanScrollUp: Boolean;
begin
  Result := FViewportOffset.Y < 0;
end;

procedure TigChannelViewer.CheckLayout;
begin
  // update WorkSize
...
function TigChannelViewer.DoMouseWheelDown(Shift: TShiftState;
...
function TigChannelViewer.DoMouseWheelUp(Shift: TShiftState;
  MousePos: TPoint): Boolean;
begin
  Result := CanScrollUp;
...
procedure TigChannelViewer.DoPaintBuffer;
var
  i, y, LMaxY : Integer;
  LRect       : TRect;
...
function TigChannelViewer.GetAlphaChannelIndexAtXY(AX, AY: Integer): Integer;
var
  LYActual         : Integer;
...
function TigChannelViewer.GetAlphaChannelPanelRect(
...
function TigChannelViewer.GetAlphaChannelPanelSnapshot(
...
function TigChannelViewer.GetAlphaChannelValidAreaForSnapshot: TRect;
var
...
function TigChannelViewer.GetColorChannelIndexAtXY(AX, AY: Integer): Integer;
var
  LYActual           : Integer;
...
function TigChannelViewer.GetColorChannelPanelRect(
  const AIndex: Integer): TRect;
begin
  Result := Rect(0, 0, 0, 0);
...
function TigChannelViewer.GetLayerMaskChannelPanelRect: TRect;
begin
...
function TigChannelViewer.GetQuickMaskChannelPanelRect: TRect;
begin
  Result := Rect(0, FViewportOffset.Y, Self.ClientWidth - 1, 0);
...
function TigChannelViewer.MoveAlphaChannelInList(
  const ACurIndex, ANewIndex: Integer): Boolean;
var
...
begin
  ...
  if Assigned(FChannelManager) then
  begin
    LChannel       := TigAlphaChannel(FChannelManager.AlphaChannelList.Channels[ACurIndex]);
    LNewLayerIndex := TigAlphaChannel(FChannelManager.AlphaChannelList.Channels[ANewIndex]).ChannelLayer.Index;

    // move channel panel first ...
    Result := FChannelManager.AlphaChannelList.Move(ACurIndex, ANewIndex);
...
function TigChannelViewer.IsRectInViewport(const ARect: TRect): Boolean;
begin
  Result := Windows.PtInRect(Self.ClientRect, ARect.TopLeft) or
            Windows.PtInRect(Self.ClientRect, ARect.BottomRight);
end;

procedure TigChannelViewer.PreparePanelSnapshotRendering(
...
begin
  ...
  begin
    ...
    LChannel    := TigAlphaChannel(FChannelManager.AlphaChannelList.Channels[LPanelIndex]);
...
procedure TigChannelViewer.Resize;
var
  LHeight : Integer;
  LDelta  : Integer;
begin
...
procedure TigChannelViewer.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  LIndex               : Integer;
  LPanelRect           : TRect;
...
procedure TigChannelViewer.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  LValidArea : TRect;
begin
  FCurrentX := X;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FScrollThread := TigScrollChannelPanelThread.Create(Self);
        end;
...
procedure TigChannelViewer.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  LIndex               : Integer;
  LChannel             : TigCustomChannel;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        MoveAlphaChannelInList(FMovingAlphaPanelIndex, LIndex);
...
function TigChannelViewer.PointOnLayerMaskPanel(AX, AY: Integer): Boolean;
var
  LYActual           : Integer;
...
function TigChannelViewer.PointOnQuickMaskPanel(AX, AY: Integer): Boolean;
var
  LYActual           : Integer;
...
procedure TigChannelViewer.Scroll(Dy: Integer);
var
  LHeight : Integer;
begin
...
function TigChannelViewer.ScrollChannelPanelInViewport(
...
// callbacks
procedure TigChannelViewer.ScrollHandler(Sender: TObject);
begin
...
function TigChannelViewer.ScrollSelectedChannelPanelInViewport: Boolean;
var
  LIndex     : Integer;
  LPanelRect : TRect;
begin
...
procedure TigChannelViewer.ScrollThreadStop;
begin
...
procedure TigChannelViewer.SetChannelManager(
  const AValue: TigCustomChannelManager);
begin
...
constructor TigScrollChannelPanelThread.Create(
  AChannelViewer: TigChannelViewer);
begin
...
procedure TigScrollChannelPanelThread.Execute;
var
  LTopRect    : TRect;
  LBottomRect : TRect;
begin

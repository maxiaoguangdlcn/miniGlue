...
type
  ...
  public
    ...
    procedure LoadFromStream(AStream: TStream; ALayerList: TigLayerList); override;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LLayer := TigNormalLayer.Create(ALayerList, LBmp.Width, LBmp.Height,
                                      $00000000, ALayerList.Count = 0);

      LLayer.LayerBitmap.Assign(LBmp);
      LLayer.UpdateLayerThumbnail;

      // The SimpleAdd() will simply add a layer to a layer list,
      // but won't calling any other functions, such as functions for
      // blending layers, callbacks, etc.
      ALayerList.SimpleAdd(LLayer);
    end;
  end;
end;
